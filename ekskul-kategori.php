...
try {
    ...
    $stmtCat = $pdo->prepare("SELECT * FROM ekskul_categories WHERE slug = ?");
    ...
    $stmtPhotos = $pdo->prepare("SELECT * FROM ekskul_photos WHERE category_id = ? ORDER BY created_at DESC");
...
$page_title = 'Ekskul ' . $category['name'];
$is_home = false;
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
  ...
  <title>Ekskul <?= htmlspecialchars($category['name']) ?> - SDIT AL FATAH</title>
    ...
    <section class="breadcrumbs">
      <div class="container">
        <div class="d-flex justify-content-between align-items-center">
          <h2>Kategori Ekskul: <?= htmlspecialchars($category['name']) ?></h2>
          <ol>
        ...
        <div class="row g-0" data-aos="fade-left">
          <?php if (empty($photos)): ?>
            <div class="col-12 text-center py-5 text-muted">
                ...
                <p>Belum ada foto dalam kategori ekskul ini.</p>
                <a href="ekskul.php" class="btn btn-brand btn-sm mt-3"><i class="fa-solid fa-arrow-left me-1"></i> Kembali ke Ekskul</a>
            </div>
          <?php else: ?>
            <?php
            $delay = 100;
            foreach ($photos as $photo):
                ...
                $alt = ($photo['caption'] !== '' && $photo['caption'] !== null)
                  ? $photo['caption']
                  : 'Ekskul SDIT Al Fatah - ' . $category['name'];
            ?>
