        ...
        if ($file_error !== UPLOAD_ERR_OK) {
            ...
            if ($check === false) {
                ...
                if (!in_array($mime, $allowed_mimes)) {
                    ...
                    $image_name = 'ekskul_' . time() . '_' . uniqid() . '.' . $ext;
...
try {
    $stmtCategories = $pdo->query("SELECT id, name FROM ekskul_categories ORDER BY name ASC");
    $categories = $stmtCategories->fetchAll();
} catch (PDOException $e) {
    $categories = [];
}

// Fetch all ekskul photos
try {
    $stmt = $pdo->query("SELECT p.*, c.name as category_name FROM ekskul_photos p LEFT JOIN ekskul_categories c ON p.category_id = c.id ORDER BY p.created_at DESC");
...
<head>
    ...
    <title>Kelola Ekskul - SDIT Al Fatah</title>
    <!-- Bootstrap 5 CSS -->
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Foto Ekstrakurikuler</h4>
                    <p class="text-muted small mb-0">Unggah dan kelola foto dokumentasi kegiatan ekskul sekolah</p>
                </div>
            </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-4">
                    <div class="card border-0 rounded-4 shadow-sm p-4 sticky-lg-top" style="top: 30px; z-index: 10;">
                        <h5 class="fw-bold text-dark mb-3"><i class="fa-solid fa-cloud-arrow-up text-success me-2"></i> Unggah Foto Ekskul</h5>
                        ...
                        <form action="" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label for="category_id" class="form-label fw-semibold text-secondary">Kategori Ekskul</label>
                ...
                <div class="col-lg-8">
                    <div class="card border-0 rounded-4 shadow-sm p-4">
                        <h5 class="fw-bold text-dark mb-4"><i class="fa-solid fa-images text-success me-2"></i> Koleksi Foto Ekskul</h5>
                        
                        <?php if (empty($ekskul_list)): ?>
                            <div class="text-center py-5 text-muted">
                                <i class="fa-regular fa-images fa-4x mb-3 text-secondary text-opacity-30"></i>
                                <p class="mb-0">Belum ada foto kegiatan ekskul. Mulai unggah di panel sebelah kiri.</p>
                            </div>
                        <?php else: ?>
                            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                                <?php foreach ($ekskul_list as $photo): ?>
                                    <div class="col">
                                        <div class="gallery-card">
                                            <div class="gallery-img-wrapper">
                                                <?php if (file_exists('../uploads/' . $photo['image'])): ?>
                                                    <img src="../uploads/<?= htmlspecialchars($photo['image']) ?>" alt="Ekskul Image" class="gallery-img">
                                                <?php else: ?>
