<?php
// admin/facility-categories/list.php
require_once '../includes/auth.php';
require_role('admin'); // Hanya admin & superadmin
require_once '../config/db.php';

try {
    // Ambil kategori beserta jumlah fasilitas didalamnya
    $stmt = $pdo->query("SELECT c.*, COUNT(f.id) as facility_count FROM facility_categories c LEFT JOIN facilities f ON c.id = f.category_id GROUP BY c.id ORDER BY c.name ASC");
    $categories = $stmt->fetchAll();
} catch (PDOException $e) {
    die("Error fetching facility categories: " . $e->getMessage());
}
...
<head>
    ...
    <title>Kategori Fasilitas - SDIT Al Fatah</title>
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Kategori Fasilitas</h4>
                    <p class="text-muted small mb-0">Kelola pengelompokan kategori fasilitas sekolah</p>
                </div>
                                ...
                                <?php if (empty($categories)): ?>
                                    <tr>
                                        <td colspan="6" class="text-center py-5 text-muted">
                                            ...
                                            <p class="mb-0">Belum ada kategori fasilitas.</p>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-2.5 py-1">
                                                    <?= (int) $cat['facility_count'] ?> Item
                                                </span>
                                            </td>
                                             <td class="pe-4 text-center">
                                                <div class="d-inline-flex gap-2">
                                                    <a href="../facilities/add.php?category_id=<?= (int)$cat['id'] ?>" class="btn btn-sm btn-outline-success" title="Tambah Fasilitas ke Kategori ini"><i class="fa-solid fa-plus"></i></a>
                                                    ...
                                                    <form action="delete.php" method="POST" onsubmit="return confirm('Hapus kategori ini? Item fasilitas di dalamnya akan dipindahkan ke kategori Umum.')">
