<?php
// admin/ekskul-categories/delete.php
...
try {
    // Ambil info kategori
    $stmtSelect = $pdo->prepare("SELECT * FROM ekskul_categories WHERE id = ?");
    ...
    $stmtUmum = $pdo->prepare("SELECT id FROM ekskul_categories WHERE slug = 'umum' LIMIT 1");
    $stmtUmum->execute();
    $umumId = $stmtUmum->fetchColumn();
    
    // Pindahkan foto yang ada di kategori ini ke kategori Umum
    if ($umumId) {
        $stmtMove = $pdo->prepare("UPDATE ekskul_photos SET category_id = ? WHERE category_id = ?");
