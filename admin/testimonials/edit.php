...
<body>
            ...
            <div class="card border-0 rounded-4 shadow-sm p-4" style="max-width:720px;">
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="hidden" name="photo_position" id="photo_position_input" value="<?= htmlspecialchars($photo_position) ?>">
                    </div>
                    <div class="mb-3">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const existingPhoto = "<?= !empty($existing_photo) ? '../uploads/' . htmlspecialchars($existing_photo) : '' ?>";
        const existingPosition = "<?= htmlspecialchars($photo_position) ?>"; // format "X% Y%"

        if (existingPhoto) {
            dragImage.src = existingPhoto;
            ...
            let pctY = 50;
            if (existingPosition) {
                const parts = existingPosition.split(' ');
                if (parts.length === 2) {
                    pctX = parseFloat(parts[0]) || 50;
                    pctY = parseFloat(parts[1]) || 50;
                }
            }
            photoPositionInput.value = `${pctX}% ${pctY}%`;
            photoPositionLabel.textContent = `${pctX}% ${pctY}%`;
        }
        ...
        dragImage.addEventListener('load', function() {
            ...
            // Calculate starting offsets based on existing or default percentage
            let pctX = 50;
            let pctY = 50;
            if (photoPositionInput.value) {
                const parts = photoPositionInput.value.split(' ');
                if (parts.length === 2) {
                    pctX = parseFloat(parts[0]) || 50;
                    pctY = parseFloat(parts[1]) || 50;
                }
            ...
            imgLeft = (diffX * pctX) / 100;
            imgTop = (diffY * pctY) / 100;
