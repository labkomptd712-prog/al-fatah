...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        if (empty($error)) {
            try {
                // Ensure slug is unique but ignore current row
                ...
                $error = "Gagal mengupdate database: " . $e->getMessage();
            }
        }
    }
}
...
<head>
    ...
    <title>Edit Berita - SDIT Al Fatah</title>
    <!-- Bootstrap 5 CSS -->
...
<body>
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Edit Berita</h4>
                    <p class="text-muted small mb-0">Ubah konten berita sekolah yang sudah diterbitkan</p>
                </div>
                <div>
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="col-lg-4">
                                    ...
                                    <div class="mb-3">
                                        <div class="border border-2 border-dashed border-secondary border-opacity-25 rounded-3 p-3 bg-white d-flex flex-column align-items-center justify-content-center" style="min-height: 180px;">
                                            <?php if (!empty($existing_image) && file_exists('../uploads/' . $existing_image)): ?>
                                                <img id="imagePreview" src="../uploads/<?= htmlspecialchars($existing_image) ?>" alt="Pratinjau Gambar" class="img-fluid rounded mb-2 animate-fade-in" style="max-height: 150px; object-fit: cover;">
                                                <div id="uploadPlaceholder" class="text-muted py-4 d-none">
                            ...
                            <button type="submit" class="btn btn-brand w-100 py-3 rounded-3 fw-bold"><i class="fa-solid fa-floppy-disk me-2"></i> Perbarui Berita</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        // Auto Slug (Only trigger if user alters the title, but typically keep current slug unless cleared)
        ...
        imageInput.addEventListener('change', function() {
            ...
            if (file) {
                ...
                <?php if (!empty($existing_image) && file_exists('../uploads/' . $existing_image)): ?>
                    imagePreview.src = "../uploads/<?= htmlspecialchars($existing_image) ?>";
                    imagePreview.classList.remove('d-none');
                    uploadPlaceholder.classList.add('d-none');
                <?php else: ?>
                    imagePreview.src = '#';
