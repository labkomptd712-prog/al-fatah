<?php
// admin/prestasi-categories/edit.php
...
try {
    $stmt = $pdo->prepare("SELECT * FROM prestasi_categories WHERE id = ?");
    $stmt->execute([$id]);
    $category = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM prestasi_categories WHERE slug = ? AND id != ?");
        ...
        if ($stmtCheck->fetchColumn() > 0) {
                ...
                if ($file_error !== UPLOAD_ERR_OK) {
                    ...
                    if ($check === false) {
                        ...
                        if (!in_array($mime, $allowed_mimes)) {
                            ...
                            $new_cover = 'prestasi_cat_' . time() . '_' . uniqid() . '.' . $ext;
            ...
            if (empty($error)) {
                try {
                    $stmtUpdate = $pdo->prepare("UPDATE prestasi_categories SET name = ?, slug = ?, cover_image = ? WHERE id = ?");
                    $stmtUpdate->execute([$name, $slug, $cover_image, $id]); logActivity($_SESSION['admin_id'], 'update', 'kategori prestasi', $name, "Mengubah kategori prestasi '{$name}'");
...
<head>
    ...
    <title>Edit Kategori Prestasi - SDIT Al Fatah</title>
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Edit Kategori Prestasi</h4>
                    <p class="text-muted small mb-0">Ubah folder atau cover kategori prestasi</p>
                </div>
