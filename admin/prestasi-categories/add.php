<?php
// admin/prestasi-categories/add.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        // Cek duplikasi slug
        $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM prestasi_categories WHERE slug = ?");
        ...
        if ($stmtCheck->fetchColumn() > 0) {
                ...
                if ($file_error !== UPLOAD_ERR_OK) {
                    ...
                    if ($check === false) {
                        ...
                        if (!in_array($mime, $allowed_mimes)) {
                            ...
                            $cover_image = 'prestasi_cat_' . time() . '_' . uniqid() . '.' . $ext;
            ...
            if (empty($error)) {
                try {
                    $stmt = $pdo->prepare("INSERT INTO prestasi_categories (name, slug, cover_image) VALUES (?, ?, ?)");
                    $stmt->execute([$name, $slug, $cover_image]); logActivity($_SESSION['admin_id'], 'create', 'kategori prestasi', $name, "Menambahkan kategori prestasi baru '{$name}'");
...
<head>
    ...
    <title>Tambah Kategori Prestasi - SDIT Al Fatah</title>
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Tambah Kategori Prestasi</h4>
                    <p class="text-muted small mb-0">Buat folder kategori prestasi siswa baru</p>
                </div>
            ...
            <div class="card border-0 rounded-4 shadow-sm p-4" style="max-width: 600px;">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control bg-light border-0 py-2 px-3 rounded-3" id="name" name="name" value="<?= htmlspecialchars($name) ?>" placeholder="Contoh: Akademik, Olahraga, Kesenian" required>
                    </div>

                    <div class="mb-3">
                        <label for="slug" class="form-label fw-semibold text-secondary">Slug URL (Opsional)</label>
                        <input type="text" class="form-control bg-light border-0 py-2 px-3 rounded-3" id="slug" name="slug" value="<?= htmlspecialchars($slug) ?>" placeholder="Akan diisi otomatis jika dikosongkan (contoh: akademik)">
                    </div>
