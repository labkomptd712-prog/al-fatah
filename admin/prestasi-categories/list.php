<?php
// admin/prestasi-categories/list.php
require_once '../includes/auth.php';
require_role('admin'); // Hanya admin & superadmin
require_once '../config/db.php';

try {
    // Ambil kategori beserta jumlah prestasi didalamnya
    $stmt = $pdo->query("SELECT c.*, COUNT(p.id) as photo_count FROM prestasi_categories c LEFT JOIN prestasi p ON c.id = p.category_id GROUP BY c.id ORDER BY c.name ASC");
    $categories = $stmt->fetchAll();
} catch (PDOException $e) {
    die("Error fetching prestasi categories: " . $e->getMessage());
}
...
<head>
    ...
    <title>Kategori Prestasi - SDIT Al Fatah</title>
    ...
    <div class="admin-container">
        ...
        <main class="main-content">
            <div class="main-header d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-dark mb-1">Kategori Prestasi (Folder)</h4>
                    <p class="text-muted small mb-0">Kelola pengelompokan folder prestasi siswa sekolah</p>
                </div>
                                ...
                                <?php if (empty($categories)): ?>
                                    <tr>
                                        <td colspan="6" class="text-center py-5 text-muted">
                                            ...
                                            <p class="mb-0">Belum ada kategori prestasi.</p>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-2.5 py-1">
                                                    <?= (int) $cat['photo_count'] ?> Item
                                                </span>
                                            </td>
                                            <td class="pe-4 text-center">
                                                <div class="d-inline-flex gap-2">
                                                    <a href="../prestasi/add.php?category_id=<?= (int) $cat['id'] ?>" class="btn btn-sm btn-outline-success" title="Tambah Item ke Kategori Ini"><i class="fa-solid fa-plus"></i></a>
                                                    ...
                                                    <form action="delete.php" method="POST" onsubmit="return confirm('Hapus kategori ini? Item di dalamnya akan dipindahkan ke kategori Umum.')">
                                                        <input type="hidden" name="id" value="<?= (int) $cat['id'] ?>">
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" title="Hapus"><i class="fa-solid fa-trash-can"></i></button>
                                                    </form>
                                                    <?php else: ?>
                                                    <button class="btn btn-sm btn-outline-secondary opacity-50" disabled title="Kategori Default"><i class="fa-solid fa-lock"></i></button>
                                                    <?php endif; ?>
                                                </div>
                                            </td>
                                        </tr>
